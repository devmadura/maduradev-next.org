...
export function ImageUpload({ value, onChange, bucket = "images", folder = "events" }: ImageUploadProps) {
    ...
    <div className="space-y-2">
      ...
      {value ? (
        <div className="relative rounded-lg border overflow-hidden">
          ...
          <Button type="button" variant="ghost" size="icon" className="absolute top-2 right-2 bg-destructive/80 text-white hover:bg-destructive" onClick={handleRemove}>
            <X className="h-4 w-4" />
          </Button>
        </div>
      ) : (
        ...
        >
          {uploading ? (
            ...
            <>
              ...
              <p className="text-xs text-muted-foreground mt-1">PNG, JPG, GIF maksimal 5MB</p>
            </>
          )}
        </div>
      )}
